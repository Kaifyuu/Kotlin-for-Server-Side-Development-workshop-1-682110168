...
- **เราตัดสินใจ / แก้อย่างไร:** ห่อทุกจุดที่เรียก `eq`/`plus` ด้วย `with(SqlExpressionBuilder) { ... }` แล้ว compile ผ่านจริง จากนั้นเขียนเทสต์ `vote increments voteCount by 1 on each call` เรียก vote 3 ครั้งแล้วยืนยันว่า count สะสมเป็น 3 (ไม่ใช่แค่ overwrite เป็น 1 ซ้ำ) — หมายเหตุ: เทสต์นี้ยืนยันว่าค่าสะสมถูกต้องเท่านั้น ไม่ได้พิสูจน์ atomicity ภายใต้ concurrent request เพราะเรียกแบบ sequential ทีละ transaction
- **สิ่งที่ได้เรียนรู้:** compile error ไม่ใช่แค่ตัวก่อกวน บางทีมันคือสัญญาณว่า API เปลี่ยน scoping/receiver ไปทั้งชุด ต้องเข้าใจว่า "ทำไม" ผิด ไม่ใช่แค่แก้ให้ผ่านโดยไม่รู้สาเหตุ — และต้องระวังไม่ overclaim ว่าเทสต์พิสูจน์อะไรเกินกว่าที่มันเช็คจริง (ชื่อเทสต์เดิมอ้างว่า "without race" ทั้งที่เป็น sequential calls แก้ชื่อให้ตรงกับสิ่งที่ทดสอบจริง)
